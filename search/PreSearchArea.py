...
class PrecisionLand(object):
	...
	def __init__(self):
		...
		self.landing_area_min_alt = sc_config.config.get_integer('general', 'landing_area_min_alt', 1.5)
		...
		# horizontal velocity pid controller.  maximum effect is 10 degree lean
        	...
        	xy_i = sc_config.config.get_float('general','VEL_XY_I',0.0)
       	 	xy_d = sc_config.config.get_float('general','VEL_XY_D',2.0)
		...
		self.point1_lon=(0.166180-0.163977)/4+149.163977
		self.point2_lon=(0.166180-0.163977)*3/4+149.163977
		self.point3_lon=(0.166180-0.163977)*3/4+149.163977
		self.point4_lon=(0.166180-0.163977)/4+149.163977

		self.point1_lat=-(0.363517-0.362616)/3-35.362616
		self.point2_lat=-(0.363517-0.362616)/3-35.362616
		self.point3_lat=-(0.363517-0.362616)*2/3-35.362616
		self.point4_lat=-(0.363517-0.362616)*2/3-35.362616
		self.point=1;
	...
	def move_to_target(self,target_info,attitude,location):
		...
		#convert to distance
		X, Y, Xp, Yp = self.pixel_point_to_position_xy((x,y),location.alt)

		#convert to world coordinates
		target_heading = math.atan2(Yp,Xp) % (2*math.pi)
	...
	def pixel_point_to_position_xy(self,pixel_position,distance):
		...
		# get time since last time velocity pid controller was run
        	dt = self.vel_xy_pid.get_dt(200.0)

        	# calculate thetaX correction and final X movement
        	thetaX_correction = self.vel_xy_pid.get_pid(thetaX, dt)

        	# calculate thetaX correction and final X movement
        	thetaY_correction = self.vel_xy_pid.get_pid(thetaY, dt)


		xp = distance * math.tan(math.radians(thetaX_correction))
		yp = distance * math.tan(math.radians(thetaY_correction))
		print xp,yp
		return (x,y,xp,yp)
